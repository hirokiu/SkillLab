...
if (float(cpu_temps[-1])) > float(120) :
    print('120度をまだ超えています')
    #メールを送る文章
    ...
    bodyText = 'パソコンの温度が120度を超えています。使用を控えましょう。温度が非常に高いです。'
...
elif (float(cpu_temps[-1])) > float(100) and float(120) > float(cpu_temps[-1]):
    print('100度を超えています')
    #メールを送る文章
    sendAddress = 'youwillbetrush@gmail.com'
    password = 'aarvvxggixzhwaal'
    ...
    subject = 'パソコンからの警告メッセージ'
    bodyText = 'パソコンの温度が100度を超えています。少し使用を控えましょう'
    fromAddress = 'youwillbetrush@gmail.com'
    toAddress = 'youwillbetrush@gmail.com'
    ...
    smtpobj.send_message(msg)
    smtpobj.close()
elif (float(cpu_temps[-1])) > float(80) and float(100) > float(cpu_temps[-1]):
    print('80度を超えています')
    #メールを送る文章
    sendAddress = 'youwillbetrush@gmail.com'
    password = 'aarvvxggixzhwaal'
    ...
    subject = 'パソコンからの警告メッセージ'
    bodyText = 'パソコンの温度が80度を超えています。温度が上がり始めているので注意しましょう'
    fromAddress = 'youwillbetrush@gmail.com'
    toAddress = 'youwillbetrush@gmail.com'
    ...
    smtpobj.send_message(msg)
    smtpobj.close()
elif (float(cpu_temps[-1])) > float(60) and float(80) > float(cpu_temps[-1]):
    print('60度を超えています')
    #メールを送る文章
    sendAddress = 'youwillbetrush@gmail.com'
    password = 'aarvvxggixzhwaal'
    ...
    subject = 'パソコンからの警告メッセージ'
    bodyText = 'パソコンの温度が60度を超えています。今のところは使っていて問題ないでしょう'
    fromAddress = 'youwillbetrush@gmail.com'
    toAddress = 'youwillbetrush@gmail.com'
    ...
    smtpobj.send_message(msg)
    smtpobj.close()
else:print("現在のCPU温度が６０度未満か実行時エラーが起きている可能性があります。")
