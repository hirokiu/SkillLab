	...
	*/
	if ((iRead = write(m_fd, &cWrite, 2)) == 2) {   // send a * to the device to get back the data
		...
		switch (iRead) {
			case -1:  
				bRet = false; // error
				fprintf(stderr, "%f: ONavi Error in read_xyz() - read(m_fd) returned %d at case -1\n", sm->t0active, iRead);
				fflush(stderr);
				break;
			case ciLen:  
				...
				if (bytesIn[ciLen] == 0x00) { // && bytesIn[0] == 0x23 && bytesIn[1] == 0x23) {
					...
					z1 = ((float) z - 45874.0f) * FLOAT_ONAVI_FACTOR * EARTH_G;
#endif
				...
				break;
			default:
				fprintf(stderr, "%f: ONavi Error in read_xyz() - read(m_fd) returned %d at case default\n", sm->t0active, iRead);
				fflush(stderr);
				x1 = x0; y1 = y0; z1 = z0;  // use last good values
		}
	}
	else {
		fprintf(stderr, "%f: ONavi Error in read_xyz() - write(m_fd) returned %d at else...\n", sm->t0active, iRead);
		fflush(stderr);
		bRet = false;
	}
